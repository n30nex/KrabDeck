...
**Current test count: 732** (1 skipped, 731 succeeded) for native_test.
...
```bash
# ⚠️ Build currently fails — see gotchas table for sigurd_mesh_v2.cpp dead code issue
...
2. `pio run -e SigurdOS_TDeck` (⚠️ **currently broken** — see gotchas: setMessageCallback dead code in sigurd_mesh_v2.cpp)
...
|| New: sigurd_mesh_v2.h split into .h/.cpp (#521) | The monolithic 1707-line `sigurd_mesh_v2.h` was split into header (`sigurd_mesh_v2.h`) and implementation (`sigurd_mesh_v2.cpp`). All inline methods were moved to the .cpp. ⚠️ Post-split, the .cpp contains a leftover `setMessageCallback()` definition whose declaration was removed in #526 — `pio run -e SigurdOS_TDeck` **fails to compile** with `no declaration matches 'void SigurdMeshV2::setMessageCallback(...)'`. |
