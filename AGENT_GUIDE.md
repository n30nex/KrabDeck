...
│   ├── spi_shared.h/cpp   # Shared SPI2_HOST (FSPI) bus singleton — LoRa + SD card share one SPIClass instance, initialized once
...
│   ├── message_store.cpp/h      # Persistent message store (SPIFFS binary ring buffer, 32 conversations, 160-char text, timestamped)
│   └── companion_adapter.inc    # Companion BLE bridge adapter — included into mesh_wrapper.cpp, PSRAM-allocated CompanionBridge (~3 KB)
├── comms/
│   ├── companion_bridge.cpp/h   # Companion BLE bridge protocol (SerialInterface transport, command enum, channel/contact/self-info types)
│   └── observed_ble_interface.cpp/h  # BLE serial observer stats & lifecycle (SIGURDOS_COMPANION_BLE compile gate)
├── ui/
│   ├── theme.h            # Colors, pixel helpers, runtime theme system (6 presets)
│   ├── theme.cpp          # Runtime theme system implementation
│   ├── responsive.h       # Display-size-agnostic layout helpers
│   ├── channel_menu.cpp/h # Alt+C channel quick-action menu (private scopes, key derivation)
...
| 22 | Bluetooth (companion BLE bridge status — enable/disable toggle, PIN display, app connection state, last sync time, compile-time gated by `SIGURDOS_COMPANION_BLE`) | `screens.cpp` | ✅ |
...
// ── Companion BLE bridge (Phase 5) ──
sigurdos::mesh::companionBleAvailable()        // BLE compiled in? (SIGURDOS_COMPANION_BLE)
sigurdos::mesh::companionBleEnabled()          // BLE advertising enabled?
sigurdos::mesh::companionBleSetEnabled(on)     // Enable/disable BLE advertising
sigurdos::mesh::companionBleConnected()        // App currently connected?
sigurdos::mesh::companionBlePin()              // BLE pairing PIN (6 digits)
sigurdos::mesh::companionBleLastSyncTime()     // Last sync epoch (0 = never)
sigurdos::mesh::companionBleLoop()             // Call from main loop
sigurdos::mesh::companionBleShutdown()         // Graceful BLE shutdown
sigurdos::mesh::messageStoreBegin()            // Init persistent message store (SPIFFS)
sigurdos::mesh::messageStoreAppend(msg)        // Write message to persistent store
sigurdos::mesh::messageStoreLoadRecent(conv, out, max) // Load recent messages for a conversation
```
...
**Current test count: ~650** (varies run-to-run, 1 skipped, all others succeed) for native_test. The count fluctuates due to dynamically generated data-driven test cases (emoji test module).
...
| `test_channel_menu` | Alt+C channel quick-action menu, private scope validation, key derivation |
...
| `test_debug` | Debug infrastructure, feature toggle macros, runtime feat selection |
| `test_buzzer` | Buzzer GPIO control and beep patterns |
| `test_telemetry_hb_ring` | Heartbeat ring buffer operations |
| `test_telemetry_packet_log` | Packet log ring buffer |
| `test_telemetry_collectors` | Telemetry data collectors (heap, PSRAM, RSSI) |
| `test_telemetry_input` | Input event telemetry (trackball, keyboard, touch) |
| `test_telemetry_crash` | Crash telemetry (reboot reason, stack watermark) |
| `test_telemetry_protocol` | Telemetry wire protocol (binary framing) |
| `test_navigation_contract` | Navigation screen enum completeness and dispatch |
| `test_github_ota_contract` | GitHub-release OTA public API contract |
| `test_mesh_contract` | Mesh wrapper public API contract completeness |
| `test_ui_contract` | UI screen public API contract |
| `test_hal_contract` | HAL public API contract |
| `test_input_contract` | Input event API contract |
| `test_prefs_defaults` | NodePrefs default values |
...
| `test_map` | Map bounding-box helpers |
| `test_lodepng_alloc` | lodepng PSRAM allocator fallback |
| `test_wifi_scan` | WiFi AP OTA scan and network discovery |
| `test_qr_show` | QR code display rendering |
| `test_onboarding` | Onboarding wizard flow |
| `test_tdeck_board` | T-Deck board power/sleep management |
...
# BLE companion bridge build (companion BLE enabled, full LoRa mesh)
pio run -e SigurdOS_TDeck_ble

# GPS hardware validation (standalone harness — boots GPS only, no display/mesh/LVGL)
...
- Tags: `beta-0.1.XX` (zero-padded for correct sort: `beta-0.1.09` not `beta-0.1.9`). Current: `beta-0.1.40`
...
| New: Companion bridge PSRAM allocation | `companion_adapter.inc` allocates the `CompanionBridge` (~3 KB) via `heap_caps_malloc(SPIRAM)` with internal-DRAM fallback. The internal DRAM .bss is nearly full — manually allocating in PSRAM avoids linker placement issues. Must use placement new and explicit destructor call on shutdown. |
| New: message_store SPIFFS binary format | `message_store.cpp` uses a raw binary ring buffer in SPIFFS (`/msg_store.bin`) with magic/version header (`MESSAGE_STORE_VERSION=1`). No cross-version compatibility — changing the `StoredMessage` struct layout silently corrupts the store. Must call `messageStoreClear()` on version mismatch. |
| New: companion_adapter.inc inclusion model | `companion_adapter.inc` is `#include`-d from `mesh_wrapper.cpp` (not compiled standalone). It accesses `mesh_wrapper`'s private statics (`rtc_clock`, `msg_buf`, `msg_count`, etc.) without widening the public API. Cannot be tested in isolation — needs the full `mesh_wrapper.cpp` translation unit. |
| New: BLE compile-time guard | `SIGURDOS_COMPANION_BLE` controls the entire BLE subsystem at compile time. When not defined, `companionBleAvailable()` returns false, the Bluetooth screen shows "Not built", and all BLE API calls are no-ops. The `SigurdOS_TDeck_ble` build env sets this flag. |
| New: Bluetooth screen uses show_screen() | The Bluetooth screen uses `show_screen()` (like Custom RF), not `navigate_to()`. It does not enter the nav stack — `go_back()` returns to whatever screen was active before entry. No persistent state between visits. |
| New: MAX_CONTACTS heap allocation (64→350) | Contacts array changed from fixed `Contact[64]` to heap-allocated `Contact*` with `MAX_CONTACTS=350` (#459). Any code assuming fixed-size array (`sizeof(contacts)/sizeof(contacts[0])`) or direct pointer arithmetic on the array will break. Buffer size must be passed explicitly to `exportContacts()`/`exportContactsFull()`. |
| New: Channel menu private scopes `$name` | Alt+C channel quick-action menu uses `$name` prefix for private scope creation (#461). Private scope key is derived via FNV-1a 64-bit (not cryptographically secure — deterministic per `sigurdos::ui::derive_private_scope_key()` namespace seed). Public `#name` channels are rejected as private scopes. |
| New: Companion sent-flag NVS persistence | Companion message sent-flags are now persisted in NVS (#457) to prevent re-send on reboot. Companion adapter state (`CompanionBridge`) survives power cycles — the sent-flag check happens during `companionAdapterLoop()` on reconnect. |
| New: GPS validation firmware (standalone env) | `SigurdOS_TDeck_gps_validation` builds only `validation/gps_validation.cpp` + `hal/gps.cpp` (#453). No display, mesh, SD, WiFi, LVGL, or buzzer startup. Boots directly into GPS NMEA logging — used for hardware bring-up. |
|| New: GPS GGA coordinate validation | `gps.cpp` now validates GGA coordinate sanity (latitude ±90°, longitude ±180°, valid fix quality ≥ 1) before accepting position fixes (#453). Corrupted or out-of-range NMEA sentences are rejected as invalid coordinates. |
|| New: GPS opt-in default (gps_enabled=false) | `gps_enabled` now defaults to `false` in prefs load (opt-in GPS). Fresh devices no longer auto-enable GPS — must be explicitly turned on via Settings → GPS. The `prefs_defaults` test was updated accordingly. |
||| New: sigurd_mesh_v2.h split into .h/.cpp (#521) | The monolithic 1707-line `sigurd_mesh_v2.h` was split into header (`sigurd_mesh_v2.h`) and implementation (`sigurd_mesh_v2.cpp`). All inline methods were moved to the .cpp. ⚠️ The `.cpp` initially contained a leftover `setMessageCallback()` definition from the old V1 age that was removed in #526 — any code path still referencing V1 types or callbacks will fail to compile. `pio run -e SigurdOS_TDeck` now **builds successfully** with the dead code removed. |
|| New: NVS string field safety (#529) | All `NodePrefs::getString()` calls now have consistent return-value guards — empty/missing strings don't produce garbage in caller buffers. |
|| New: strcpy→strncpy sweep (#528) | All `strcpy` calls replaced with `strncpy` + manual null-termination across the codebase for bounds safety. |
|| New: onAnonDataRecv packet buffer fix (#527) | `onAnonDataRecv` no longer mutates the packet buffer in-place; anonymous name entropy increased to 20 chars for privacy. |
|| New: delay(0)→delay(1) for TWDT feeding (#525) | All `delay(0)` calls in tile scan loops replaced with `delay(1)` to feed the Task WatchDog Timer during long SD card reads. |
|| New: multibyte path hash support (#533) | Adverts and messages now support 1/2/3-byte path hash encoding for flood scoping — reduces airtime for short paths. |
|| New: Shared SPI bus singleton (#546) | `src/hal/spi_shared.h/cpp` provides a single `SPIClass(FSPI)` instance shared by the LoRa radio and SD card via `sigurdos_shared_spi_begin()`/`sigurdos_shared_spi()`. LovyanGFX still manages its own bus internally. The singleton prevents double bus initialisation. Do not include `<SPI.h>` in other headers — it breaks native test builds. |
|| New: Auto-advert interval in NVS (#537, #538) | `advert_interval_h` (`uint16_t` in `NodePrefs`) controls periodic adverts: 0=disabled, 24/72/168 hours. Implemented in `mesh_wrapper.cpp` main loop — one advert per interval period, not continuous. Changing via Settings → Network takes effect on next loop iteration (no reboot needed). Silently drains battery if left at 24h — no UI prompt about power impact. |
|| New: I2C clock tug-of-war fixed (#559) | Both GT911 (touch) and keyboard poll the shared I2C bus on every loop. Previously each set conflicting speeds (400kHz vs 100kHz). Now both use a compromise 200kHz set once at init — all per-loop `Wire.setClock()` calls removed. The 200kHz speed is functional for both devices. |
|| New: Battery ADC calibrated (#545) | `analogReadMilliVolts` replaces `analogRead()` for efuse-calibrated ADC on ESP32-S3. Old `sigurdos_battery_mv_from_adc_raw()` removed. Battery voltage = ADC pin mV × 2 (2:1 voltage divider on T-Deck). Eliminates 10-24% voltage error from uncalibrated raw reads. |
|| New: Trackball center click INPUT_PULLUP (#554) | GPIO 0 (trackball center click) changed from bare `INPUT` to `INPUT_PULLUP` matching all other trackball pins. Prevents phantom clicks on T-Deck variants without external pull-up (closes #549). |
|| New: Factory reset uses nvs_flash_erase() (#557) | `factoryReset()` now calls `nvs_flash_erase()` to completely wipe all NVS namespaces (not just the `mesh` namespace). Ensures truly clean factory state. |
|| New: SignalSample key increased to 4 bytes (#556) | The NVS key for signal history samples increased from 2 to 4 bytes to prevent RSSI/SNR aliasing when multiple contacts are stored. Old NVS data with 2-byte keys silently lost after upgrade — signal history resets for all contacts. |
|| New: DM name stack buffer safety (#543, #553) | Direct message name formatting now uses properly sized stack buffers to prevent overflow on long contact names. |
|| New: Channel message buffer clamped (#561) | `append_channel_message()` now clamps stored message count per channel to actual allocated buffer size instead of assuming a fixed maximum — prevents out-of-bounds writes. |
|| New: PIN entry blocks trackball (#558) | Trackball events are ignored during PIN entry (Bluetooth pairing, security PIN prompt) to prevent back-swipe gesture from bypassing the lock. |
|| New: Crash handler backtrace limitation (#562) | `telemetry_crash.cpp` has a documented limitation that `esp_panic_handler` backtrace capture cannot yet recover full register context on ESP32-S3 — only PC/SP are captured. |
|| New: Forward channel messages to room servers (#539) | Channel messages are now forwarded to logged-in room servers so the server maintains conversation history on behalf of clients. |
...
|||- **Since last sync:** sigurd_mesh_v2.h split into .h/.cpp (#521), multibyte path hash support (1/2/3-byte adverts & messages, #533), strcpy→strncpy safety sweep (#528), NVS string field return-value guards (#529), fix: onAnonDataRecv packet buffer mutation + anon name entropy increase (#527), fix: remove dead onMeshMessage/_message_cb callback code (#526), fix: delay(0)→delay(1) for TWDT feeding in tile scan loops (#525), GPS gps_enabled default changed to false (opt-in GPS), beta-0.1.40 (RC2) release, duration-limited auto-advert 24h/72h/168h (#537, #538), forward channel messages to room servers (#539), guest permission display fix (#540), shared SPI bus singleton (#546), calibrated battery ADC via analogReadMilliVolts (#545), trackball center click INPUT_PULLUP (#554), NVS zero-init repeater password (#555), SignalSample 4-byte key fix (#556), factory reset nvs_flash_erase() (#557), PIN entry trackball block (#558), I2C clock tug-of-war fix 200kHz (#559), DM name stack buffer safety (#543/#553), append_channel_message capacity clamp (#561), crash handler backtrace limitation (#562), and `test_build_info` + `test_contact_paging` test modules for ~650 total test cases.
