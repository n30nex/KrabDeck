...
- Send one immediate flood advert after each production boot using the latest
  ...
  RF-off.
- On every trusted `main` push, build and validate the exact candidate bytes
  without opening serial devices, flashing hardware, admitting evidence,
  approving an environment, tagging, or publishing.
- Run exact-device qualification, evidence admission, signing and publication
  only through an explicit protected manual release operation.
- **Gate:** a manually qualified edge release plus an injected failure proving
  exactly one retry and automatic RF-off recovery.
...
- Run a two-hour active soak for each release candidate under the authoritative
  hardware thresholds.
- **Gate:** exact-SHA physical evidence, no open P0/P1 defects, and protected
  manual approval of signed `v1.0.0` publication.
