...
address the stable USB identity loaded from the runner-private configuration.
Importing this module and running its unit tests never touches a serial device.
"""
...
def redact_text(text: str) -> str:
    ...
    try:
        ...
        return redacted
    for value in (
        str(config.target_by_id),
        *(str(path) for path in config.forbidden_by_id),
        config.expected_usb_serial,
        config.expected_efuse_mac,
        config.expected_properties["ID_PATH"],
        *config.forbidden_serials,
    ):
...
class DeviceGuard:
    ...
    def validate_properties(
        properties: Mapping[str, str], config: FixtureConfig | None = None
    ) -> None:
        ...
        if vid_pid in config.forbidden_vid_pids:
            raise SafetyError("D1L USB identity is explicitly forbidden")
        if properties.get("ID_SERIAL_SHORT", "").upper() in config.forbidden_serials:
    ...
    def inspect(self, *, check_busy: bool = True) -> DeviceSnapshot:
        ...
        matches = [
            path
            for path in config.target_by_id.parent.iterdir()
            if path.is_symlink() and config.expected_usb_serial in path.name.upper()
        ]
        if matches != [config.target_by_id]:
        ...
        for forbidden in config.forbidden_by_id:
            if self._same_device(config.target_by_id, forbidden):
        ...
        self.validate_properties(properties, config)

        if check_busy:
            busy = self.runner.run(
                ["fuser", str(config.target_by_id)], timeout=10, check=False
            )
    ...
    def validate_efuse(self, esptool: "EspTool") -> str:
        ...
        for before in ("no-reset", "usb-reset"):
            try:
                ...
                last_error = error
                continue
            if mac != config.expected_efuse_mac:
...
def execute_release(
    ...
    *,
    ...
    # Load the runner-private identity before constructing any hardware path.
    fixture_config()
