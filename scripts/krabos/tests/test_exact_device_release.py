...
TEST_CONFIG = release.FixtureConfig.from_mapping(
    {
        ...
        "forbidden_devices": [
            {
                "by_id": "/dev/serial/by-id/test-d1l",
                "serial": "66:77:88:99:AA:BB",
                "vid_pid": ["1a86", "7523"],
            },
            {
                "by_id": "/dev/serial/by-id/test-peer-CC:DD:EE:FF:00:11",
                "serial": "CC:DD:EE:FF:00:11",
                "vid_pid": ["10c4", "ea60"],
            },
        ],
    }
)
release.fixture_config = lambda: TEST_CONFIG
...
class FakeGuard:
    def __init__(self) -> None:
        self.snapshot = release.DeviceSnapshot(
            "/dev/ttyACM2", dict(TEST_CONFIG.expected_properties), release.utc_now()
        )
    ...
    def validate_efuse(self, _: object) -> str:
        return TEST_CONFIG.expected_efuse_mac
...
class FakeEspTool:
    ...
    def read_mac(self, *, before: str) -> str:
        return f"MAC: {TEST_CONFIG.expected_efuse_mac}"
...
class IdentityGuardTests(unittest.TestCase):
    def test_exact_properties_pass(self) -> None:
        release.DeviceGuard.validate_properties(dict(TEST_CONFIG.expected_properties))

    def test_d1l_and_peer_are_explicitly_rejected(self) -> None:
        d1l = dict(TEST_CONFIG.expected_properties)
        ...
        peer = dict(TEST_CONFIG.expected_properties)
        peer["ID_SERIAL_SHORT"] = next(
            iter(TEST_CONFIG.forbidden_serials - {"66:77:88:99:AA:BB"})
        )
    ...
    def test_id_path_is_part_of_the_identity(self) -> None:
        moved = dict(TEST_CONFIG.expected_properties)
    ...
    def test_esptool_always_uses_the_pinned_by_id_path(self) -> None:
        runner = RecordingRunner(
            release.CommandResult(0, f"MAC: {TEST_CONFIG.expected_efuse_mac}", "")
        )
        tool = release.EspTool(runner, "/venv/bin/python")
        tool.read_mac(before="no-reset")
        command = runner.calls[0]
        self.assertEqual(
            command[command.index("--port") + 1], str(TEST_CONFIG.target_by_id)
        )
        self.assertNotIn(str(TEST_CONFIG.forbidden_by_id[0]), command)
        self.assertNotIn(str(TEST_CONFIG.forbidden_by_id[1]), command)
...
class OrchestratorTests(unittest.TestCase):
    ...
    def test_target_only_diagnostics_fail_closed_and_restore_recovery(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertNotIn(
                TEST_CONFIG.expected_usb_serial, public.read_text(encoding="utf-8")
            )
            if os.name == "posix":
