...
class BuildMetadataContractTests(unittest.TestCase):
    ...
    def test_release_provenance_is_invariant_across_actions_retries(self):
        ...
        helper = next(
            ...
            if isinstance(node, ast.FunctionDef) and node.name == "_actions_provenance"
        )
        namespace = {}
        exec(compile(ast.Module(body=[helper], type_ignores=[]),
                     "scripts/build_metadata.py", "exec"), namespace)
        provenance = namespace["_actions_provenance"]
        sha = "a" * 40
        first = provenance("v1.0.0", sha, "https://github.com", "n30nex/KrabDeck",
                           "100", "1", "main")
        retry = provenance("v1.0.0", sha, "https://github.com", "n30nex/KrabDeck",
                           "999", "7", "different-ref")
        self.assertEqual(first, retry)
        self.assertEqual(first, (
            "commit-aaaaaaaaaaaa", "", sha,
