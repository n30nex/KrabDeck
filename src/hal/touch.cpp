...
static constexpr size_t   GT911_CONFIG_SIZE    = 186;
...
// ── Config dump (186 bytes) ───────────────────────────────
static const uint8_t gt911_default_config[GT911_CONFIG_SIZE] = {
    // Generated by GT911 config tool — standard T-Deck settings
    // Header (0x8047-0x8053, 13 bytes)
    0x42, 0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x41,
    // The rest of the config is read from chip after reset and written back
    // with any changes. We read the existing config first, then optionally
    // tweak it.
};
...
bool slopos_touch_init()
{
    ...
    // Read existing config from chip
    uint8_t config[GT911_CONFIG_SIZE];
    if (i2c_read_bytes(GT911_REG_CONFIG, config, sizeof(config))) {
        // Write it back (required by GT911 init sequence)
        i2c_write_bytes(GT911_REG_CONFIG, config, sizeof(config));
    }
