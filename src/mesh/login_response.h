...
inline Parsed parse(const uint8_t* data, uint8_t len)
{
    ...
    // Current MeshCore servers return exactly 13 bytes: timestamp, result,
    // keep-alive, permissions, ACL, random nonce, and firmware level.
    ...
    if (data[4] == 0) {
        ...
        result.acl_permissions = data[7];
        result.firmware_level = data[12];
    } else {
